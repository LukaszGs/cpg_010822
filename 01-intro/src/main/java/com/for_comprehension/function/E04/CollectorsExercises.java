...
class CollectorsExercises {
    ...
    static Function<List<String>, List<String>> L3_unmodifiable() {
        return list -> {
            ...
            //                .collect(Collectors.collectingAndThen(toList(), Collections::unmodifiableList)); JDK8
        };
    }
    ...
    static Function<List<String>, Map<String, Integer>> L4_toMap() {
        return list -> list.stream()
            .collect(toMap(String::toUpperCase, String::length, anyValue()));
    }
    ...
    static Function<List<String>, TreeMap<String, Integer>> L5_toTreeMap() {
        return list -> list.stream()
            .collect(toMap(String::toUpperCase, String::length, anyValue(), TreeMap::new));
    }
    ...
    static Function<List<String>, Map<Integer, String>> L9_groupStrings() {
        return input -> input.stream()
            .collect(groupingBy(String::length, collectingAndThen(toSet(), set -> String.join(",", set))));
    }

    private static <T> BinaryOperator<T> anyValue() {
        return (i, i2) -> i;
    }
